...
train$Attendance<-NULL

#Extract columns for modeling
model_train<-train[,.(W, L, FPI, `Stadium Capacity`, PD)]
model_test<-test[,.(W,L, FPI, `Stadium Capacity`, PD)]
...
#Train the model
XGB_model<-xgb.train(params=param,nrounds= 711,missing=NA,data=dtrain,watchlist=watchlist,
...
test$Error<- as.numeric(test$Error)
